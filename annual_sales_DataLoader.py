...
# #各月店數
# store_number_path= r'D:/斑鳩/營業店年度營業額及各項比率計算/各月店家數.xlsx'
# #檔案輸出位置
# output_folder_path= r'D:\斑鳩\code'



# store_name_list = ['文化店', '新田店', '光華店', '中華店', '愛河店', '時代店', '義大店', '草衙店', '民族店', '新楠店', '驛站店', '中山店', '仁德店', '西門店', '博愛店', '公園店', '鹽行店', '豐原店', '新仁店', '斗南店', '市政店', '嘉福店', '林口店', '三重店', '土城店', '舊宗店', '汐止店', '新店店', '淡新店']

# all_data_dict = {store_name: {m+1: {} for m in range(month)} for store_name in store_name_list}


# index_list = ["銷貨收入", "食材銷貨收入", "雜項銷貨收入", "公務費收入", "其他收入", "其他支出", "收入合計",
#     "費用", "費用率", "費用率\n(不含公務費)", "實際毛利", "實際毛利率(%)", "單位淨利", "淨利率", "應發薪資",
#     "薪資比", "奬金", "奬金比", "年終奬金",   "奬金比(年終)", "現場人員薪資", "工讀生薪資比\n(佔食材進貨金額)",
#     "工時生產力", "薪資生產力"]

# columns_list = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二', '合  計']    

# # 建立空的 DataFrame
# center_kitchen_df = pd.DataFrame(index=index_list, columns=columns_list) 
...
# 'output_folder_path' : r'D:\斑鳩\code'
# }
# d = DataLoader(Config, store_name_list, all_data_dict, center_kitchen_df)



# all_data_dict = d.load_all()
# rent_df = d.rent_df

# center_kitchen_df = d.load_center_kitchen_all()
# same_period_profit_dict = d.load_same_period_profit_dict()
# same_period_profit_dict = d.same_period_profit_dict
